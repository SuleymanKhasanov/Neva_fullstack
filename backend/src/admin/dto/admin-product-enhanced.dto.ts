...
export class AdminProductsEnhancedService {
  ...
  async findAll() {
    ...
    return products.map(this.formatProductEnhanced);
  }
  ...
  async create(createProductDto: CreateProductEnhancedDto) {
    // Валидация данных
  ...
  async update(id: number, updateProductDto: UpdateProductEnhancedDto) {
  ...
  private async validateProductDataEnhanced(dto: CreateProductEnhancedDto) {
    // Проверить категорию
  ...
  private formatProductEnhanced(product: any) {
    return {
      id: product.id,
